...
ore_users_count_dict = defaultdict(int)
def get_ore(ele, needed):
    ...
    for chem in ingredients:
        if chem[1] in ore_sources:
            ...
            ore_users_count_dict[chem[1]]+= 1
        else:
            get_ore(chem[1], chem[0] * need)

get_ore('FUEL', 1)
pprint(ore_users_dict.items())
pprint(ore_users_count_dict.items())
ore = 0
for k,v in ore_users_dict.items():
    ...
    #ore_used = netx value up from net_ore that is a mult of _map[k][0]
